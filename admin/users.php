...
function user_list()
{
    ...
    for ($i=0; $i<$count; $i++) {
        ...
        //var_dump();
        $user_list[$i]['team_total'] = team_total($user_list[$i]['user_id']);
    }
...
function team_total($user_id){
    $c = 0;
    $q[]='';
    $sql = "SELECT user_id from ".$GLOBALS['ecs']->table('users')." WHERE parent_id = ".$user_id;
    $q = $GLOBALS['db']->getAll($sql);
    $c += count($q);
    var_dump($user_id);
    
    for($i=0;$i<count($q);$i++){
        var_dump("for--".$q[$i]['user_id']);
        $sql1 = "SELECT user_id from ".$GLOBALS['ecs']->table('users')." WHERE parent_id = ".$q[$i]['user_id'];
        $m = $GLOBALS['db']->getAll($sql1);
        $c += count($m);
        while($m){
